...
- [ ] Blade-mode cuts: draw the seam anywhere instead of picking across or up. Wanted sooner rather than later (2026-08-17): a drawn line says exactly where the cut lands, where the current buttons cut at the last tap and slide off any hold in the way, which is hard to aim. Cuts must not cross each other, or the vertex where they meet cannot stay watertight. Designed 2026-09-01 as ADR-010 (geometry) and ADR-011 (gesture), with undo (ADR-012) first. Geometry landed 2026-09-03: faces are convex outlines with a seam-edge reference, collision runs on convex prisms, `cutFaceAlong` takes any straight seam, saved walls migrate to document version 2; ACROSS and UP still make the seams. The drawn gesture, trim and the cut animation are what is left.
