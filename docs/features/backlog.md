...
- [ ] Editor surface rework: canvas owns the screen, controls come to the selection (popovers on panel and hold), collapsible tool rail, status strip, per-panel wall colour, curated swatch sets. Shaped 2026-08-17: PRD `docs/prd/editor-surface.md`. Driven by the sidebar already being crowded at eleven controls with generation, difficulty, save/load and resizing still queued for the same column.
