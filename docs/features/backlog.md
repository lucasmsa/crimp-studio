...
- [x] Blade-mode cuts (2026-09-03, ADR-010, ADR-011): BLADE and TRIM tools. Press a panel and drag; the seam runs through the press point and the cursor out to the panel's border both ways, drawn on the plywood with a degree chip at the cursor end, ink where it can be cut and red where it cannot. Release commits; a tap selects the panel; Escape cancels. Two refusals: a seam through a hold, a seam through an edge a child panel hinges on. BLADE hinges the far piece on the seam, which opens a few degrees and settles while its seam flashes. TRIM throws the far piece away with its holds and every panel hinged on it, hatched and tinted during the drag, falling away on release; undo brings it back. ACROSS, UP and the seam-nudging findCutPosition are gone. Faces are convex outlines with a seam-edge reference and collision runs on convex prisms (ADR-010); saved walls migrate to document version 2.
- [x] Undo and redo (2026-09-02, ADR-012): Cmd/Ctrl+Z and Shift+Cmd+Z or Ctrl+Y, plus header buttons. Wall document only, on zundo with a marker per edit so measurements stay out of history and a held arrow key is one entry. A hold history removes pops off instead of vanishing.
